...
namespace Scanner.Livox
{
    class LivoxDataUtil
    {
        ...
        private readonly UInt16[] crc_table_mcrf4xx = new UInt16[]{
        ...
        // crc32 table
        private readonly UInt32[] crc_table_crc32 = new UInt32[]{
        ...
        private UInt16 seed_16 = 0x4c49;

        private UInt32 seed_32 = 0x564f580a;

        /*
        unsafe
        public UInt16 mcrf4xx_calc_1(byte* data,int len) {

            UInt16 crc = seed_16;

            Console.WriteLine("{0:x}",(int)data);

            while (((int)data & 3) != 0 && len > 0)
            {
                crc = Convert.ToUInt16((crc >> 8) ^ crc_table_mcrf4xx[(crc & 0xff) ^ *(data++)]);
                len--;
            }

	        while (len >= 16) {
		        len -= 16;
		        crc_n4(ref crc, ((UInt32*) data)[0], crc_table_mcrf4xx);
                crc_n4(ref crc, ((UInt32*) data)[1], crc_table_mcrf4xx);
                crc_n4(ref crc, ((UInt32*) data)[2], crc_table_mcrf4xx);
                crc_n4(ref crc, ((UInt32*) data)[3], crc_table_mcrf4xx);
                data += 16;
	        }

	        while (len-->0) {
		        crc = (UInt16)((crc >> 8) ^ crc_table_mcrf4xx[(crc & 0xff) ^ *(data++)]);
	        }
	        return crc;
        }*/

        public bool CheckPreamble(byte[] data) {
            int length = Marshal.SizeOf(typeof(SdkPreamble));

            UInt16 crc = this.mcrf4xx_calc(data, length);

            return crc == 0;
        }

        public bool CheckPacket(byte[] data) {
            UInt32 crc = BitConverter.ToUInt32(data, data.Length - 4);
            UInt64 calc_crc = this.crc32_calc(data, data.Length - 4);
            return crc == calc_crc;
        }

        public SdkPacket ParsePacket(byte[] data) {
            SdkPacket packet = DataConvert.ConvertByteStruct<SdkPacket>(data);

            int packet_data_length = packet.length - DataConvert.GetLength<SdkPacket>() + 1;

            packet.data = new byte[packet_data_length];

            Buffer.BlockCopy(data, data.Length - packet_data_length - 4, packet.data, 0, packet_data_length);

            packet.crc = BitConverter.ToUInt32(data, data.Length - 4);

            return packet;
        }

        public UInt16 mcrf4xx_calc(byte[] data, int length){
            UInt16 crc = seed_16;
            ...
            for(int i=0;i<length;i++){
                ...
                crc = Convert.ToUInt16((crc >> 8) ^ crc_table_mcrf4xx[(crc & 0xff) ^ data[i]]);
            ...
            while(length >= 16){
                length -= 16;
                crc_n4(ref crc, BitConverter.ToUInt32(data, pre_index), crc_table_mcrf4xx);
                pre_index += 4;
                crc_n4(ref crc, BitConverter.ToUInt32(data, pre_index), crc_table_mcrf4xx);
                pre_index += 4;
                crc_n4(ref crc, BitConverter.ToUInt32(data, pre_index), crc_table_mcrf4xx);
            ...
            while(length--!=0){
                crc = (UInt16)((crc >> 8) ^ crc_table_mcrf4xx[(crc & 0xff) ^ data[pre_index++]]);
            }
            return crc;
        }

        public void crc_n4(ref UInt16 crc, UInt32 data, UInt16[] table)
        {
            crc = Convert.ToUInt16(crc ^ data);
            crc = Convert.ToUInt16(table[(crc & 0xff) + 0x300] ^ table[((crc >> 8) & 0xff) + 0x200] ^ table[((data >> 16) & 0xff) + 0x100] ^ table[data >> 24]);
        }

        public void crcsm_n4d(ref UInt32 crc,UInt32 data,UInt32[] table) {
            crc = Convert.ToUInt32(crc^data);
            crc = Convert.ToUInt32((crc >> 8)^table[crc & 0xff]);
            crc = Convert.ToUInt32((crc >> 8)^table[crc & 0xff]);
	        crc = Convert.ToUInt32((crc >> 8)^table[crc & 0xff]);
	        crc = Convert.ToUInt32((crc >> 8)^table[crc & 0xff]);
        }

        public UInt32 crc32_calc(byte[] data,int length){
            UInt32 crc = seed_32 ^ 0xffffffff;
            ...
            while (length >= 16){
                length -= 16;
                crcsm_n4d(ref crc, BitConverter.ToUInt32(data,pre_index), crc_table_crc32);
                pre_index += 4;
                crcsm_n4d(ref crc, BitConverter.ToUInt32(data,pre_index), crc_table_crc32);
                pre_index += 4;
                crcsm_n4d(ref crc, BitConverter.ToUInt32(data,pre_index), crc_table_crc32);
            ...
            while(length--!=0){
                crc = Convert.ToUInt32((crc >> 8) ^ crc_table_crc32[(crc & 0xff)^ data[pre_index++]]);
            }
        ...
        private UInt16 desired = 1;
        public UInt16 GenerateSeq() {
            if (desired == UInt16.MaxValue) {
                desired = 1;
            }
            return desired++;
        }
        ...
        public int GetPacketLength() {
            return Marshal.SizeOf(typeof(SdkPacket)) - 1;
        }

        public int GetPreambleLen() {
            return Marshal.SizeOf(typeof(SdkPreamble));
        }

        public byte[] PackCommand(SdkPacket packet) {
            byte[] result = new byte[packet.length];

            byte[] data = DataConvert.ConvertStructByte<SdkPreamble>(packet);
            packet.preamble_crc = this.mcrf4xx_calc(data,data.Length-2);

            data = DataConvert.ConvertStructByte<SdkPacket>(packet);

            Buffer.BlockCopy(data, 0, result, 0, this.GetPacketLength() - 4);

            if (packet.data != null) {
                Buffer.BlockCopy(packet.data, 0, result, this.GetPacketLength() - 4, packet.data.Length);
            }

            packet.crc = this.crc32_calc(result, result.Length - 4);

            byte[] crc_bytes = BitConverter.GetBytes(packet.crc);

            Buffer.BlockCopy(crc_bytes, 0, result, result.Length-4, crc_bytes.Length);

            return result;
            //return DataConvert.ConvertStructByte<Sdk(packet);
        }

        public SdkPacket PackSdkPacket(byte packet_type,byte cmd_set, byte cmd_id, byte[] data){
            SdkPacket packet;
            packet.sof = 0xAA;
            packet.version = 1;
            packet.packet_type = packet_type;
            packet.seq_num = this.GenerateSeq();
            packet.length = Convert.ToUInt16(this.GetPacketLength() + (data!=null?data.Length:0));
            packet.cmd_set = cmd_set;
            packet.cmd_id = cmd_id;
            packet.data = data;
            packet.crc = 0;
            packet.preamble_crc = 0;

            return packet;
        }

        public IPAddress GetIPAddress(int index=0) {
            IPAddress address = null;
            string name = Dns.GetHostName();
            IPAddress[] address_list = Dns.GetHostAddresses(name);

            if (address_list.Length > index){
        ...
        public string GetGateway(){
            ...
            foreach (var netWork in nics)
            {
                ...
                foreach(var gateWay in ip.UnicastAddresses)
                {

                    Console.WriteLine(gateWay.Address);
                    Console.WriteLine(gateWay.IPv4Mask);
                    /*
                    //如果能够Ping通网关
